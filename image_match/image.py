...
def locate_more_image_Template(origin, query, outfile, num=0):
    ...
    if num == 0:
        ...
        for i in range(num):            
            maxval, maxloc = template_match(img2, img1, [], 0)
            if DEBUG: print "835: ", maxval
            if maxval < 0.9844: return None
            center.append([int(maxloc[0]+w/2),int(maxloc[1]+h/2)])
            _region_pixel_zero(img2, center[-1], w, h)
...
def locate_one_image_Multi_Template(origin, query, outfile):
    _path_check(origin)
    _path_check(query)
    img1 = cv2.imread(query, 0)  # queryImage,gray
    img2 = cv2.imread(origin, 0)  # originImage,gray
    ...
    target_img = cv2.imread(origin, 1)  # originImage
    [height, width] = [img1.shape[0], img1.shape[1]]
    [tar_height, tar_width] = [img2.shape[0], img2.shape[1]]
    if tar_width < tar_height:
        rotate_img2 = _image_rotate(img2, 90, scale=1.)
        match_value_or, match_posi_or, ratio_or = multi_scale_match(img2, img1)
        match_value_r, match_posi_r, ratio_r = multi_scale_match(rotate_img2,img1)
        if match_value_or < match_value_r:
            match_value, posi, ratio = match_value_r, match_posi_r, ratio_r
            match_posi = [0,0]
            [match_posi[0],match_posi[1]] = [tar_width-posi[1]-int(height*ratio),posi[0]]
            [h, w] = [int(width*ratio), int(height*ratio)]
        else:
            match_value, match_posi, ratio = match_value_or, match_posi_or, ratio_or
            [h, w] = [int(height*ratio), int(width*ratio)]
    else:
        match_value, match_posi, ratio = multi_scale_match(img2, img1)
        [h, w] = [int(height*ratio), int(width*ratio)]
    if 0.85 < match_value:
        center = [0.0,0.0]
        [center[0], center[1]] = [int(match_posi[0]+w/2), int(match_posi[1]+h/2)]
        _image_rectangle(target_img, [center], w, h,outfile)
        return center
    else:
        return None
...
if __name__ == '__main__':
    ...
    datadir = 'D:/match/dataset'
    outputdir = 'D:/match/results_combined2'
    querydir = 'D:/match/query'
    for name in os.listdir(querydir):
        query_path = os.path.join(querydir,name)
        query_name = name.split('.')[0]
        outfile_path = os.path.join(outputdir,query_name)
        if os.path.exists(outfile_path) == 0:
            os.mkdir(outfile_path)
        print outfile_path
        out_txt = query_name + '.txt'
        txt_path = os.path.join(outfile_path,out_txt)
        query_txt = open(txt_path,'a')
        for lists in os.listdir(datadir):
            print lists
            file_path = os.path.join(datadir,lists)
            print file_path
            outfile = os.path.join(outfile_path,lists)
            starttime1 = time.clock()
            pts = locate_image(file_path, query_path, outfile, 0.3)
            endtime1 = time.clock()
            if pts: 
                print pts
                time1 = endtime1 - starttime1
                query_txt.write(lists+': ')
                query_txt.write(str(time1))
                query_txt.write('\n')
    #for name in os.listdir(outfile_path):
        #os.remove(os.path.join(outfile_path,name))
    #os.rmdir(outfile_path)
    #pts = locate_image('testdata/target.png', 'testdata/query.png',
                        #'testdata/DEBUG.png', 0.3)
    #pts = locate_one_image_SIFT('testdata/target.jpg', 'testdata/query.png',
                        #'testdata/DEBUG.png', 0.3)                        
